<?php
/**
 * Mur Puts och Betong I Borås AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Mur Puts och Betong I Borås AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through mu_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'murputsochbetongibora', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'murputsochbetongibora' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'murputsochbetongibora' ) ) );

		register_block_pattern_category( 'murputsochbetongibora-sections', array( 'label' => __( 'Mur Puts och Betong I Borås AB sections', 'murputsochbetongibora' ) ) );
		register_block_pattern_category( 'murputsochbetongibora-pages', array( 'label' => __( 'Mur Puts och Betong I Borås AB page layouts', 'murputsochbetongibora' ) ) );
	}
);
