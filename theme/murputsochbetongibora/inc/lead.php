 ...
 * or photo. They call mu_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Mur Puts och Betong I Borås AB.
 *
 ...
 * @return string
 */
function mu_sample_note() {
	return mu_lead( 'ar_riktig' ) ? '' : ' <span class="mu-sample">(platshållare)</span>';
}
 ...
 * @return string File name for mu_lead_img().
 */
function mu_lead_bild( $i ) {
	$bilder = (array) mu_lead( 'bilder', array() );
 ...
 * @return string
 */
function mu_ring_label() {
	$namn = mu_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, mu_lead( 'telefon' ) ) : 'Ring ' . mu_lead( 'telefon' );
}
