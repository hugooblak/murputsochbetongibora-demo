...
add_filter(
	...
	function ( $exporters ) {
		$exporters['murputsochbetongibora-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'murputsochbetongibora-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'murputsochbetongibora-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'murputsochbetongibora-leads',
						'group_label' => __( 'Quote requests', 'murputsochbetongibora-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['murputsochbetongibora-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'murputsochbetongibora-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
