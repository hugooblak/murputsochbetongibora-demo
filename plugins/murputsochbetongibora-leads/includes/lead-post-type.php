...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'murputsochbetongibora-leads' ),
					'singular_name' => __( 'Lead', 'murputsochbetongibora-leads' ),
					'edit_item'     => __( 'Förfrågan', 'murputsochbetongibora-leads' ),
					'search_items'  => __( 'Search leads', 'murputsochbetongibora-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'murputsochbetongibora-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'murputsochbetongibora-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'murputsochbetongibora-leads' ),
			'npl_phone'    => __( 'Phone', 'murputsochbetongibora-leads' ),
			'npl_service'  => __( 'Needs', 'murputsochbetongibora-leads' ),
			'npl_timeline' => __( 'When', 'murputsochbetongibora-leads' ),
			'npl_zip'      => __( 'ZIP', 'murputsochbetongibora-leads' ),
			'npl_source'   => __( 'Source', 'murputsochbetongibora-leads' ),
			'npl_status'   => __( 'Status', 'murputsochbetongibora-leads' ),
			'date'         => __( 'Received', 'murputsochbetongibora-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'murputsochbetongibora-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'murputsochbetongibora-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'murputsochbetongibora-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'murputsochbetongibora-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'murputsochbetongibora-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'murputsochbetongibora-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'murputsochbetongibora-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'murputsochbetongibora-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'murputsochbetongibora-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'murputsochbetongibora-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'murputsochbetongibora-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'murputsochbetongibora-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
